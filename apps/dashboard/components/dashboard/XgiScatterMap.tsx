...
/**
 * XgiScatterMap — expected goals and assists plotted against expected minutes.
 *
 * Two SVG charts share the same gameweek cursor: xG vs minutes and xA vs
 * minutes. Dots are coloured by position. A companion table lists the top ten
 * players sorted by the selected field.
 *
 * Scatter math mirrors KFT2627/assets/xgi-scatter.js.
 */
...
// ── Position colours (matches KFT2627) ───────────────────────────────────────
...
// ── Axis helpers (mirrors KFT2627/assets/xgi-scatter.js) ─────────────────────
...
type SortField = "xgi" | "xg" | "xa" | "xpts" | "minutes";
...
function buildDotRows(rows: ProjectionRow[], gw: number): DotRow[] {
  return rows.flatMap((r) => {
    ...
    const xg =
      typeof r.xg === "number" && Number.isFinite(r.xg) ? r.xg : null;
    const xa =
      typeof r.xa === "number" && Number.isFinite(r.xa) ? r.xa : null;
    if (minutes === null || xg === null || xa === null) return [];
    return [{
...
function ScatterChart({
  ...
  const tipRow = tip?.row ?? null;
  const yVal = tipRow ? tipRow[yKey] : null;
  ...
  return (
    <div className="relative">
      <svg
        ...
        className="w-full"
        role="img"
        aria-label={`${yLabel} against expected minutes`}
      ...
      >
        ...
        {tip && (
          <g className="pointer-events-none">
            <line
              x1={M.l} x2={tip.cx} y1={tip.cy} y2={tip.cy}
              stroke="rgba(127,127,127,.35)" strokeDasharray="3 3" strokeWidth={1}
            />
            <line
              x1={tip.cx} x2={tip.cx} y1={tip.cy} y2={M.t + CH}
              stroke="rgba(127,127,127,.35)" strokeDasharray="3 3" strokeWidth={1}
            />
          </g>
        )}
        ...
        {rows.map((r) => {
          ...
          return (
            <circle
              ...
              tabIndex={0}
              role="button"
              aria-label={`${r.name} (${r.team ?? "?"}, ${r.position ?? "?"}) · Mins ${r.minutes.toFixed(0)} · ${yLabel} ${yVal.toFixed(2)}`}
        ...
        >
          <p className="flex items-center gap-1.5 text-xs font-semibold text-white">
            <span
              ...
              style={{ backgroundColor: POSITION_COLOR[tipRow.position ?? ""] ?? "#7A8799" }}
            />
            {tipRow.name}
          </p>
          <p className="mt-0.5 text-[10px] text-zinc-400">
            {tipRow.team ?? "—"} · {tipRow.position ?? "—"}
            {tipRow.price !== null && ` · £${tipRow.price.toFixed(1)}`}
          </p>
          <dl className="mt-1.5 grid grid-cols-2 gap-x-3 gap-y-0.5 font-mono text-[10px]">
            <dt className="text-zinc-400">xG</dt><dd className="text-right text-white">{tipRow.xg.toFixed(2)}</dd>
            <dt className="text-zinc-400">xA</dt><dd className="text-right text-white">{tipRow.xa.toFixed(2)}</dd>
            <dt className="text-zinc-400">xGI</dt><dd className="text-right text-white">{tipRow.xgi.toFixed(2)}</dd>
            {tipRow.xpts !== null && <><dt className="text-zinc-400">xPts</dt><dd className="text-right text-white">{tipRow.xpts.toFixed(2)}</dd></>}
            <dt className="text-zinc-400">Mins</dt><dd className="text-right text-white">{tipRow.minutes.toFixed(0)}</dd>
          </dl>
        </div>
      )}
    </div>
...
export default function XgiScatterMap({
  rows,
  gameweeks,
}: {
  rows: ProjectionRow[];
  ...
  // Clamp index so a stale cursor never goes out of bounds if gameweeks changes.
  ...
  const xgMax = useMemo(() => axisMax(filtered.map((r) => r.xg)), [filtered]);
  const xaMax = useMemo(() => axisMax(filtered.map((r) => r.xa)), [filtered]);
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="border border-border bg-card">
        ...
        {/* Row 1: GW nav + position toggles + status */}
      ...
      <div className="border border-border bg-card">
        <div className="overflow-x-auto px-3 pb-1 pt-3">
          {filtered.length === 0 ? (
            ...
            <>
              <ScatterChart
                rows={filtered} yKey="xg" yLabel="Expected goals (xG)"
                xMax={xMax} yMax={xgMax} hovered={hovered} onHover={setHovered}
              />
              <ScatterChart
                rows={filtered} yKey="xa" yLabel="Expected assists (xA)"
                xMax={xMax} yMax={xaMax} hovered={hovered} onHover={setHovered}
              />
            </>
          )}
        </div>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-border text-left text-[11px] font-semibold text-muted-foreground">
                ...
                <th className="px-3 py-2 text-right">xGI</th>
            ...
            <tbody>
              {topRows.map((r) => (
                ...
                >
                  <td className="px-3 py-1.5">
                    <span className="inline-flex items-center gap-1.5">
                      ...
                      <span
                        ...
                        style={{ backgroundColor: POSITION_COLOR[r.position ?? ""] ?? "#7A8799" }}
                        aria-hidden
                      />
                      {r.name}
                    </span>
                  ...
                  <td className="px-3 py-1.5 text-right font-mono tabular-nums">{r.xg.toFixed(2)}</td>
                  <td className="px-3 py-1.5 text-right font-mono tabular-nums">{r.xa.toFixed(2)}</td>
                  <td className={cn("px-3 py-1.5 text-right font-mono tabular-nums font-semibold", sort === "xgi" && "text-primary")}>{r.xgi.toFixed(2)}</td>
